...
import { Button } from './index';
import { NFTContext } from '../context/NFTContext';
import OCButton from './OCButton.tsx'; // Import the OCButton component
    ...
    <ul className={`list-none flexCenter flex-row ${isMobile && 'flex-col h-full'}`}>
      {['Explore NFTs', 'Listed NFTs', 'My NFTs'].map((item, i) => (
        ...
        >
          <Link href={generateLink(i)}>{item}</Link>
          {item === 'Explore NFTs' && (
            <OCButton onClick={() => alert('connected')}>
              connect
            </OCButton>
          )}
        </li>
    ...
    <nav className="flexBetween w-full fixed z-10 p-4 flex-row border-b dark:bg-nft-dark bg-white dark:border-nft-black-1 border-nft-gray-1">
      <div className="flex flex-1 flex-row justify-start">
        <Link href="/">
          <div className="flexCenter cursor-pointer md:hidden">
            <Image src={images.logo02} alt="logo" width={32} height={32} />
            <p className="dark:text-white text-nft-black-1 font-semibold text-lg ml-1">Crypt-expo</p>
          </div>
        </Link>
        <Link href="/">
          <div
